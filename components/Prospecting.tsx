...
import { Search, MapPin, Briefcase, Globe, Phone, Mail, Plus, Loader2, UserPlus, Building2, MessageCircle, ArrowDownCircle } from 'lucide-react';
...
const Prospecting: React.FC<ProspectingProps> = ({ onAddLead, currentUser }) => {
  ...
  const handleLoadMore = async () => {
    ...
    try {
        // Envia os nomes atuais para a IA excluir da nova busca
        const currentNames = results.map(r => r.name);
        const newProspects = await findProspects(niche, region, currentNames);
        
        // Filtra duplicatas caso a IA retorne algum repetido mesmo assim
        const uniqueNew = newProspects.filter(np => !results.some(r => r.name === np.name));
        
        if (uniqueNew.length === 0) {
            alert("A IA não encontrou novos resultados distintos para esta busca no momento.");
        } else {
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500">
      ...
      {/* Search Header */}
      <div className="bg-[#1e2e41] p-6 rounded-2xl border border-[#1687cb]/20 mb-6 shadow-lg shrink-0">
        ...
        {isLoading ? (
            ...
            <>
                <div className="flex justify-between items-center mb-4 px-2">
                    <h4 className="text-slate-400 text-sm font-bold uppercase">{results.length} Empresas encontradas</h4>
                </div>

                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4 mb-6">
                    {results.map((prospect, index) => (
                        <div key={index} className="bg-[#1e2e41] p-5 rounded-2xl border border-[#1687cb]/10 hover:border-[#20bbe3]/40 transition-all group shadow-md flex flex-col h-full animate-in fade-in slide-in-from-bottom-4" style={{ animationDelay: `${(index % 10) * 50}ms` }}>
                ...
                <div className="flex justify-center pb-6">
                    <button 
                        ...
                        className="bg-[#1e2e41] hover:bg-[#1687cb]/20 text-[#20bbe3] border border-[#20bbe3]/30 px-6 py-3 rounded-full font-bold transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed hover:scale-105 active:scale-95"
                    >
                        {isLoadingMore ? (
                            <>
